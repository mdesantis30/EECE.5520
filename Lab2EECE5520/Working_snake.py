...
# Noted the serial port dev file name, based on the particular host machine
# Initializing serial port
serialDevFile = 'COM3'
ser=serial.Serial(serialDevFile, 9600, timeout=0) # should it be timeout=1? 
...
# Keyboard bindings (Removed the keyboard bindings for snake control since we'll be controlling it via joystick)
# wn.listen()
# wn.onkey(go_up, "w")
# wn.onkey(go_down, "s")
# wn.onkey(go_left, "a")
...
while True:
    ...
    if control_information == 'Moves down':
        head.direction = "down"
    elif control_information == 'Moves up':
        head.direction = "up"
    elif control_information == 'Moves right':
        head.direction = "right"
    ...
    if head.distance(food) < 20:
        ...
        # Send a flag to Arduino indicating an apple is eaten
        ser.write(b'e')
